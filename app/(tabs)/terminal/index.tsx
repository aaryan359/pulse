...
function TerminalModal({ server, onClose }: { server: Server | null; onClose: () => void }) {
	const [input, setInput] = useState("");
	const [logs, setLogs] = useState<string[]>(["Connecting to server...", "Authenticated successfully.", "Welcome to the terminal."]);
	...
	const runCommand = () => {
		...
		setLogs((prev) => [...prev, `$ ${input}`, "command output will appear here"]);
		setInput("");
	};
		...
		<Modal
			...
			<SafeAreaView style={{ flex: 1, backgroundColor: "#020617" }}>
				...
				<View style={styles.terminalHeader}>
					<View>
						<Text style={{ color: "#e5e7eb", fontWeight: "600" }}>{server.hostname}</Text>
						<Text style={{ color: "#9ca3af", fontSize: 12 }}>Connected • {server.os}</Text>
					</View>
